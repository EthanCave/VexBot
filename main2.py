...
model1 = joblib.load('gradient_boosting_model.pkl')
...
def get_team_id(team_number):
    ...
    search_url = f"https://www.robotevents.com/api/v2/teams?number%5B%5D={team_number}&myTeams=false&program%5B%5D=1"
...
async def events(ctx, *args):
    args = args[0] if args else db.reference(f"{ctx.message.author.id}/Team").get()
    try:
...
async def info(ctx, *args):
    arg = args[0] if args else db.reference(f"{ctx.message.author.id}/Team").get()
...
async def help(ctx):
    ...
    embed.add_field(name=".resources", value="Shows helpful Vex resources", inline=False)
    embed.add_field(name=".setteam", value="Sets your team number. Uses this number for commands if no team_number is given.", inline=False)
    embed.add_field(name=".myteam", value="Shows your team number", inline=False)
...
async def myteam(ctx):
    try:
        Team  = db.reference(f"{ctx.message.author.id}/Team").get()
        await ctx.send(f"You are part of team {Team}")
    except:
        await ctx.send("You have not given a Team previously!")
...
@bot.command()
async def matchup(ctx, team1: str, team2: str, team3: str, team4: str):
    try:
       ...
       # Predict win probabilities using all three models
        predicted_win_probability1 = model1.predict(np.array(new_team_data))
        predicted_win_probability2 = model2.predict(np.array(new_team_data))
        predicted_win_probability3 = model3.predict(np.array(new_team_data))
        
        # Aggregate probabilities for each alliance for each model
        blue_alliance_prob1 = np.mean(predicted_win_probability1[:2])
        red_alliance_prob1 = np.mean(predicted_win_probability1[2:])
        
        blue_alliance_prob2 = np.mean(predicted_win_probability2[:2])
        red_alliance_prob2 = np.mean(predicted_win_probability2[2:])
        
        blue_alliance_prob3 = np.mean(predicted_win_probability3[:2])
        red_alliance_prob3 = np.mean(predicted_win_probability3[2:])
        
        # Normalize probabilities for each model
        total_prob1 = blue_alliance_prob1 + red_alliance_prob1
        blue_alliance_prob_normalized1 = blue_alliance_prob1 / total_prob1
        red_alliance_prob_normalized1 = red_alliance_prob1 / total_prob1
        
        total_prob2 = blue_alliance_prob2 + red_alliance_prob2
        blue_alliance_prob_normalized2 = blue_alliance_prob2 / total_prob2
        red_alliance_prob_normalized2 = red_alliance_prob2 / total_prob2
        
        total_prob3 = blue_alliance_prob3 + red_alliance_prob3
        blue_alliance_prob_normalized3 = blue_alliance_prob3 / total_prob3
        red_alliance_prob_normalized3 = red_alliance_prob3 / total_prob3
        print(blue_alliance_prob_normalized1, blue_alliance_prob_normalized2, blue_alliance_prob_normalized3)
        print(red_alliance_prob_normalized1, red_alliance_prob_normalized2, red_alliance_prob_normalized3)
        # Prepare and send the predictions as a message
        embed = discord.Embed(title="Matchup Predictions", color=discord.Color.blue())
        # Check if all models predict blue alliance to win
        # Check if all models predict blue alliance to win
        if blue_alliance_prob_normalized1 > 0.5 and blue_alliance_prob_normalized2 > 0.5 and blue_alliance_prob_normalized3 > 0.5:
            overall_prediction = f"{team_names[2]} & {team_names[3]} are strongly favored"
            avg_win_percent = (blue_alliance_prob_normalized1 * 100 + blue_alliance_prob_normalized2 * 100 + blue_alliance_prob_normalized3 * 100) / 3
        # Check if all models predict red alliance to win
        elif red_alliance_prob_normalized1 > 0.5 and red_alliance_prob_normalized2 > 0.5 and red_alliance_prob_normalized3 > 0.5:
            overall_prediction = f"{team_names[0]} & {team_names[1]} are strongly favored"
            avg_win_percent = (red_alliance_prob_normalized1 * 100 + red_alliance_prob_normalized2 * 100 + red_alliance_prob_normalized3 * 100) / 3 
        # Check if two models agree and one disagrees
        elif (blue_alliance_prob_normalized1 > 0.5 and blue_alliance_prob_normalized2 > 0.5) or (red_alliance_prob_normalized1 > 0.5 and red_alliance_prob_normalized2 > 0.5):
            # Determine the favored team based on which models agree
            print((blue_alliance_prob_normalized1 > 0.5)  + (blue_alliance_prob_normalized2 > 0.5) + (blue_alliance_prob_normalized3 > 0.5))
            if ((blue_alliance_prob_normalized1 > 0.5)  + (blue_alliance_prob_normalized2 > 0.5) + (blue_alliance_prob_normalized3)) >= 2:
                favored_team = f"{team_names[0]} & {team_names[1]}"
                avg_win_percent = (blue_alliance_prob_normalized1 + blue_alliance_prob_normalized2 + blue_alliance_prob_normalized3) / 3 * 100
            else:
                favored_team = f"{team_names[2]} & {team_names[3]}"
                avg_win_percent = (red_alliance_prob_normalized1 + red_alliance_prob_normalized2 + red_alliance_prob_normalized1) / 3 * 100
            overall_prediction = f"{favored_team} is slightly favored"
        # If none of the above conditions are met, no clear advantage for any alliance
        else:
            overall_prediction = "No clear advantage for any alliance"
            avg_win_percent = None 

        if avg_win_percent is not None:
            avg_win_percent_rounded = round(avg_win_percent, 2)
        else:
            avg_win_percent_rounded = None

        embed.add_field(name="Prediction", value=f"{overall_prediction}" + f" ({avg_win_percent_rounded}%)" if avg_win_percent_rounded is not None else f"{overall_prediction}")
        await ctx.send(embed=embed)
